...
tag: [3d-array, Dynamic Programming, Generator]
toc: true
toc_sticky: true
...
def is_edge(cur_x, cur_y, nxt_x, nxt_y, rectangle):
    x, y = (cur_x + nxt_x)/2, (cur_y + nxt_y)/2
    is_onEdge = any((x in (x1,x2) and y1<=y<=y2) or (y in (y1,y2) and x1<=x<=x2) for x1,y1, x2, y2 in rectangle)
    is_inEdge = any((x1<x<x2 and y1<y<y2) for x1,y1,x2,y2 in rectangle)
    return is_onEdge and not is_inEdge

def solution(rectangle, characterX, characterY, itemX, itemY):
    cur = (characterX, characterY)
    prev = None

    for dist in itertools.count():  # 무한반복+count
        if cur == (characterX, characterY) and prev:    #prev_pos, 0빼고 다 true
            total = dist
            break

        elif cur == (itemX, itemY):
            dist_toItem = dist

        for dx, dy in ((0,1), (0,-1), (1,0), (-1,0)):
            nxt = (cur[0]+dx, cur[1]+dy)
            if prev != nxt and is_edge(*cur, *nxt, rectangle):
                prev, cur = cur, nxt
                break

    answer = min(dist_toItem, dist-dist_toItem)
    return answer
