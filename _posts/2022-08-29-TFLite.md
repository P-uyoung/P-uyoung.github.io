...
| Tensorfflow | auto  | keras.Model  | <span style="color:lightseagreen">tf.keras.utils.Sequence</span>     | keras.layers.Dense     | forward()      | model()     |

*Tensorflow는 tf.debugging.set_log_device_placement(True) 코드를 통해서
어디에 할당되어있는지를 확인할 수 있다. <br/>
PyTorch는 GPU 탑재 여부를 담은 변수 device를 모델 객체에 할당할 때 포함시킵니다. model = NeuralNetwork().to(device) <br/>
